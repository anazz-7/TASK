...
/* ---------------- VISUAL PDF REPORT EXPORTS FOR LABELS & PACKAGES ---------------- */
window.__exportLabelReportPDF = function() {
  const list = isManagerPlus() ? cache.labels : cache.labels.filter(l => l.staff_id === session.staffId);
  if (!list || !list.length) {
    if (typeof window.showToast === 'function') window.showToast('No labelling records to export!', 'error');
    return;
  }
  ...
  const uniqueItemsCount = new Set(list.map(e => e.item)).size;
  const uniqueDatesCount = new Set(list.map(e => e.date)).size;
  ...
  const printHtml = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Labelling Production Report - ${bizName}</title>
      <style>
        ...
        .stat-card { background: #FEF3C7; border: 1px solid #FDE68A; padding: 10px; border-radius: 8px; text-align: center; }
        .stat-label { font-size: 0.7rem; color: #92400E; text-transform: uppercase; font-weight: 700; }
        .stat-val { font-size: 1.2rem; font-weight: 800; color: #78350F; margin-top: 4px; }
        ...
        .badge { display: inline-block; padding: 2px 6px; border-radius: 4px; background: #E2E8F0; color: #334155; font-size: 0.7rem; font-weight: 700; }
    ...
    <body>
      <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #0F172A;padding-bottom:12px;margin-bottom:16px;">
        <div style="flex:1;">
          <h1 style="margin:0;font-size:1.5rem;text-transform:uppercase;letter-spacing:1px;color:#0F172A;font-weight:800;">THE MALABAR OILS</h1>
          <div style="font-size:0.78rem;color:#334155;margin-top:4px;line-height:1.4;">
            <div><b>Address:</b> 44 PALLIVASAL STREET VADAGARAI PERIYAKULAM, THENI DTS 625601</div>
            <div><b>Phone / Trade Enquiry:</b> +91 6379849947</div>
            <div><b>Website:</b> <a href="https://www.themalabarfarmfoods.store" target="_blank" style="color:#0284C7;text-decoration:none;">www.themalabarfarmfoods.store</a></div>
          </div>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:14px;flex-shrink:0;">
          ...
          <div style="text-align:center;">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=https://www.themalabarfarmfoods.store" alt="QR Code" style="width:76px;height:76px;border:1px solid #CBD5E1;padding:3px;border-radius:6px;background:#FFF;" />
            <div style="font-size:0.58rem;color:#64748B;font-weight:700;margin-top:2px;">SCAN WEBSITE</div>
          </div>
        </div>
      </div>

      <div class="meta-bar">
        <span><b>Report Type:</b> Labelling Production Log</span>
        <span><b>Export Date:</b> ${exportDate}</span>
        <span><b>Generated By:</b> ${esc(session.name)} (${session.role})</span>
      </div>

      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-label">Total Quantity Labelled</div>
          <div class="stat-val">${totalQty.toLocaleString('en-IN')} QTY</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Unique Products Labelled</div>
          <div class="stat-val">${uniqueItemsCount} Items</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Log Days Recorded</div>
          <div class="stat-val">${uniqueDatesCount} Days</div>
        </div>
      </div>
        ...
        <tbody>
          ${sortedList.map((item, idx) => `
            <tr>
              ...
              <td><b>${item.date}</b></td>
              <td><b>${esc(item.item || '(unnamed)')}</b></td>
              <td><span class="badge">${esc(item.batch_code || item.batch_no || 'N/A')}</span></td>
              <td>${esc(staffName(item.staff_id))}</td>
              <td class="text-right"><b>${Number(item.qty || 0).toLocaleString('en-IN')}</b></td>
              <td style="color:#64748B;">${esc(item.notes || '—')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
...
window.__exportPackageReportPDF = function() {
  const list = isManagerPlus() ? cache.packages : cache.packages.filter(l => l.staff_id === session.staffId);
  if (!list || !list.length) {
    if (typeof window.showToast === 'function') window.showToast('No packaging records to export!', 'error');
    return;
  }
  ...
  const printHtml = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Packaging Production Report - ${bizName}</title>
      <style>
        body { font-family: 'Roboto Mono', 'Inter', monospace, sans-serif; margin: 20px; color: #0F172A; background: #FFFFFF; }
        .header { text-align: center; border-bottom: 2px solid #0F172A; padding-bottom: 12px; margin-bottom: 16px; }
        .header h1 { margin: 0; font-size: 1.4rem; text-transform: uppercase; letter-spacing: 1px; color: #0F172A; }
        ...
        .meta-bar { display: flex; justify-content: space-between; background: #F8FAFC; padding: 10px 14px; border-radius: 8px; border: 1px solid #E2E8F0; margin-bottom: 16px; font-size: 0.8rem; }
        .stats-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 20px; }
        .stat-card { background: #E0E7FF; border: 1px solid #C7D2FE; padding: 10px; border-radius: 8px; text-align: center; }
        .stat-label { font-size: 0.7rem; color: #3730A3; text-transform: uppercase; font-weight: 700; }
        .stat-val { font-size: 1.2rem; font-weight: 800; color: #1E1B4B; margin-top: 4px; }
        table { width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 0.8rem; }
        th, td { border: 1px solid #CBD5E1; padding: 8px 10px; text-align: left; }
        th { background: #1E293B; color: #FFFFFF; text-transform: uppercase; font-size: 0.72rem; font-weight: 700; }
        tr:nth-child(even) { background: #F8FAFC; }
    ...
    <body>
      ...
      <div class="meta-bar">
        <span><b>Report Type:</b> Packaging Production Log</span>
        <span><b>Export Date:</b> ${exportDate}</span>
        <span><b>Generated By:</b> ${esc(session.name)} (${session.role})</span>
      </div>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-label">Total Quantity Packaged</div>
          <div class="stat-val">${totalQty.toLocaleString('en-IN')} QTY</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Unique Products Packaged</div>
          <div class="stat-val">${uniqueItemsCount} Items</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Log Days Recorded</div>
